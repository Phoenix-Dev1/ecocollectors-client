...
import {
  fetchRecyclerJoinRequests,
  updateRecyclerJoinRequestStatus,
} from './ManagerFunctions';

const RecyclersJoinRequests = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchRecyclerJoinRequests(selectedStatusFilter); // Pass the selected status filter
  ...
  const handleUpdateStatus = async (joinID, newStatus, userID) => {
    try {
      await updateRecyclerJoinRequestStatus(joinID, newStatus, userID);
...
export default RecyclersJoinRequests;
